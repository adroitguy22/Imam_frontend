...
export const CreateClassModal: React.FC<CreateClassModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const [selectedTeachers, setSelectedTeachers] = useState<{ teacherId: string, role: string }[]>([
        { teacherId: '', role: 'Islamiyyah Teacher' },
        { teacherId: '', role: 'Conventional Teacher' }
    ]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Filter out empty teacher assignments
            ...
            setError(err.response?.data?.error || 'Failed to create class. Please check inputs.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Create New Class" maxWidth="2xl">
                    ...
                    <div className="space-y-4">
                            ...
                            {selectedTeachers.map((assignment, index) => (
                                <div key={index} className="p-3 bg-gray-50 rounded-xl border border-gray-100 space-y-2 relative group">
                                    ...
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <input
                                            type="text"
                                            placeholder="Assign Role (e.g. Mentor)"
                                    ...
                                    <select
                                        ...
                                        className="w-full text-sm px-3 py-1.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none"
                                    >
                        ...
                        {selectedTeachers.length === 0 && (
                            <p className="text-xs text-gray-400 italic text-center py-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                No teachers assigned. You can assign them later.
                            </p>
                        )}
                    </div>
                </div>
